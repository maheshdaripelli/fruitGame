...
class App extends Component {
  ...
  playAgainClick = () => {
    this.setState({isGameOver: false, score: 0, tumbObj: imagesList[0]})
  }

  mainClick = id => {
    const {tumbObj} = this.state
    const random = Math.ceil(Math.random() * 25)
    console.log(random)
    if (tumbObj.id === id) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({tumbObj: imagesList[random]})
    } else {
  ...
  render() {
    const {activeTabId, tumbObj, isGameOver, score} = this.state
    const {thumbnailUrl} = tumbObj
    const filterList = this.getFilteredProjects()
    return (
      <div>
        <Header score={score} />
        <div className="appDiv">
          {isGameOver ? (
            <>
              <GameOver playAgainClick={this.playAgainClick} score={score} />
            </>
          ) : (
            <>
              <img src={thumbnailUrl} className="img3" alt="thumbnail" />
              ...
              <ul>
                {filterList.map(each => (
                  <Item each={each} key={each.id} mainClick={this.mainClick} />
                ))}
              </ul>
            </>
          )}
